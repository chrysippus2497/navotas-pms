...
<h5>Archived Project Management </h5><hr>
...
<?php 
  ...
  if(isset($_GET['a'])){
    echo '<div class="pt-2" id="success-alert"><span class="alert alert-success">Successfuly Archived Project</span></div>';
  }
?>
...
<div id="card-size" class="card mt-4 mx-auto shadow-sm bg-white rounded" style="width:70rem">
  ...
  <div class="card-body">
      ...
      <tr>
          ...
          $stmt = $db_con->query("SELECT * FROM tbl_projects WHERE status='Inactive'");
          while ($row = $stmt->fetch()) {           
            if($row['status']=="Active")
            {
              ...
              echo '<td><a type="button" id="todo-btn" class="btn btn-sm btn-outline-success" href="includes/functions.php?rp='.$row['id'].'"><i class="fas fa-undo"></i> Restore</a></td>';
